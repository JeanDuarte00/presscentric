...
@Slf4j
@Service
public class CreateUserUsecase implements IUsecase<User, User>{

    private final IUserDataFetcher dataFetcher;

    public CreateUserUsecase(IUserDataFetcher dataFetcher) {
        this.dataFetcher = dataFetcher;
    }

    @Override
    public User execute(User data) {
        var response = this.dataFetcher.create(data);
